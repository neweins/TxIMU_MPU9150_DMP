...
#define DMP_CODE_SIZE           (3062)	//12 x 16 x16
...
int dmp_read_fifo(short *gyro, short *accel, long *quat, unsigned long *timestamp, short *sensors, unsigned char *more)
{
    ...
    {
		#ifdef FIFO_CORRUPTION_CHECK
        long quat_q14[4], quat_mag_sq;	// local variables for check corruption
		#endif

        quat[0] = ((long)fifo_data[0] << 24) |
        		  ((long)fifo_data[1] << 16) |
        		  ((long)fifo_data[2] <<  8) |
        		         fifo_data[3];

        quat[1] = ((long)fifo_data[4] << 24) |
        		  ((long)fifo_data[5] << 16) |
        		  ((long)fifo_data[6] <<  8) |
        		         fifo_data[7];

        quat[2] = ((long)fifo_data[8] << 24) |
        		  ((long)fifo_data[9] << 16) |
        		  ((long)fifo_data[10] << 8) |
        		         fifo_data[11];

        quat[3] = ((long)fifo_data[12] << 24) |
        		  ((long)fifo_data[13] << 16) |
        		  ((long)fifo_data[14] <<  8) |
        		         fifo_data[15];
        ...
        {/* Quaternion is outside of the acceptable threshold. */
            ...
            sensors[0] = 0;
           	...
           	simple_uart_putstring((const uint8_t *)"\n\rError>> Q_mag square sum range (251658240 ~ 285212672) : ");
			simple_uart_putinteger10(quat_mag_sq);
			simple_uart_putstring((const uint8_t *)"\n\r");
            return -2;
        }
