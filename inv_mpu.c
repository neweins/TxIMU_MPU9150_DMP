 ...
 * nrf_delay_ms(unsigned long num_ms)
...
#define i2c_write   !mpu9150_write_Bytes
#define i2c_read    !mpu9150_read_Bytes
...
//  These variables are set by the mpu_select_device function
...
#ifdef AK89xx_SECONDARY
    reg->raw_compass      = 0x49;	//EXT_SENS_DATA_00
    reg->yg_offs_tc       = 0x01;
    reg->s0_addr          = 0x25;
    reg->s0_reg           = 0x26;
    reg->s0_ctrl          = 0x27;
...
#define MAX_COMPASS_SAMPLE_RATE (10)
#endif
...
{
    unsigned char WhoAmI, data[6], rev;
    int errCode;

    mpu9150_read_Bytes(st->hw->addr, st->reg->who_am_i, 1, &WhoAmI);
    if(WhoAmI != MPU9150_ADDRESS)
    {
		simple_uart_putstring((const uint8_t *)"\n Please, check your Device ID(expected ID:0x68)\n\r");
		return -11;
    }
    ...
    if (rev)
    {
        ...
        {
			#ifdef MPU_DEBUG
			simple_uart_putstring((const uint8_t *)"Unsupported software product rev: ");
			simple_uart_putinteger10(rev);
			simple_uart_putstring((const uint8_t *)"\n\r");
			#endif
			return -4;
        }
    ...
    else
    {
        ...
        {
			#ifdef MPU_DEBUG
        	simple_uart_putstring((const uint8_t *)"Product ID read as 0 indicates device is either incompatible or an MPU3050");
			#endif
            return -6;
        }
        else if (rev == 4)
        {
			#ifdef MPU_DEBUG
        	simple_uart_putstring((const uint8_t *)"Half sensitivity part found.");
			#endif
            st->chip_cfg.accel_half = 1;
        }
        else
    ...
    {
		#ifdef MPU_DEBUG
    	simple_uart_putstring((const uint8_t *)"Unsupported software product rev: ");  simple_uart_putinteger10(rev);
		#endif
        return -7;
    }
    ...
    st->chip_cfg.clk_src = INV_CLK_PLL;	// = 1
...
#ifdef AK89xx_SECONDARY
#ifdef MPU_DEBUG
    simple_uart_putstring((const uint8_t *)"Setting up compass.\n\r");
#endif
    errCode = setup_compass();
    if (errCode != 0)
    {
		#ifdef MPU_DEBUG
    	simple_uart_putstring((const uint8_t *)"Setup compass failed: ");
    	simple_uart_putinteger10(errCode);
    	simple_uart_putstring((const uint8_t *)"\n\r");
		#endif
    }
...
int mpu_read_fifo_stream(unsigned short length, unsigned char *data, unsigned char *more)
{
    ...
    if (!st->chip_cfg.sensors)
    {
    	simple_uart_putstring((const uint8_t *)"\n\rError>> NOT be configured sensors:");
		simple_uart_putinteger10((int32_t)st->chip_cfg.sensors);
		simple_uart_putstring((const uint8_t *)"\n\r" );
        return -2;
    }
    ...
    if (fifo_count < length)
    {
        more[0] = 0;
        simple_uart_putstring((const uint8_t *)"\n\rError>> fifo_count:");
        simple_uart_putinteger10((int32_t)fifo_count);
        simple_uart_putstring((const uint8_t *)"  length:" );
        simple_uart_putinteger10((int32_t)length);
        simple_uart_putstring((const uint8_t *)"\n\r" );
        return -4;
    }
    ...
    {/* FIFO is 50% full, better check overflow bit. */
        ...
        {
            simple_uart_putstring((const uint8_t *)"\n\rError>> FIFO Overflow occurred !\n\r");
            simple_uart_putstring((const uint8_t *)"fifo_count:" );
            simple_uart_putinteger10((int32_t)fifo_count);
            simple_uart_putstring((const uint8_t *)"  length:" );
            simple_uart_putinteger10((int32_t)length);
            simple_uart_putstring((const uint8_t *)"\n\r" );
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st->hw->addr, st->reg->fifo_en, 1, data))
        return -1;
    nrf_delay_ms((uint32_t volatile)test->wait_ms);
    data[0] = 0;
    if (i2c_write(st->hw->addr, st->reg->fifo_en, 1, data))
        return -1;
...
#define LOAD_CHUNK  (16)
    ...
    {//length = 3062
        ...
        if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii]))	return -3;

        if (mpu_read_mem(ii, this_write, cur))	return -4;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable)
    {
        ...
        tmp = 0;
        mpu9150_write_Bytes(st->hw->addr, 0x23, 1, &tmp);	// instead of i2c_write
...
static int setup_compass(void)
{
    ...
    //----START setup compass, directly-----//
    mpu_set_bypass(1);	// bypass gyro and accel
...
//    for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++)
    for (akm_addr = (0x0C)<<1; akm_addr <= (0x0F)<<1; akm_addr++)	// changed by Hari	(1/3)
    {
        int result;
        result = i2c_read(akm_addr, AKM_REG_WHOAMI, 1, data);
        if (!result && (data[0] == AKM_WHOAMI))	//Deviece ID : 0x48 fixed
            break;
    }

//    if (akm_addr > 0x0F)
    if (akm_addr > (0x0F)<<1)	//changed by Hari (2/3)
    {
        /* TODO: Handle this case in all compass-related functions. */
		#ifdef MPU_DEBUG
    	simple_uart_putstring((const uint8_t *)"Compass not found.\n\r");
		#endif
        return -1;
    }
     ...
     */
    st->chip_cfg.compass_addr = akm_addr;		//for nRF I2C dev
//    st->chip_cfg.compass_addr = akm_addr>>1;	//changed by Hari (3/3)
    /*******************************************************************/
    ...
    #ifdef MPU_DEBUG
    simple_uart_putstring((const uint8_t *)"Compass sens: ");
    simple_uart_putinteger10(st->chip_cfg.mag_sens_adj[0]); simple_uart_putstring((const uint8_t *)" ");
    simple_uart_putinteger10(st->chip_cfg.mag_sens_adj[1]); simple_uart_putstring((const uint8_t *)" ");;
    simple_uart_putinteger10(st->chip_cfg.mag_sens_adj[2]); simple_uart_putstring((const uint8_t *)"\n\r");
	#endif
    ...
    mpu_set_bypass(0);	// disable bypass gyro and accel
    //----END setup compass, directly-----//
    ...
    data[0] = BIT_I2C_READ | st->chip_cfg.compass_addr>>1;	//changed by JKlee
    ...
    data[0] = st->chip_cfg.compass_addr>>1;	//changed by JKlee
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
    ...
    if (!(tmp[0] & AKM_DATA_READY))
    {
    	simple_uart_putstring((const uint8_t *)"error>> compass_data[0] = ");
		simple_uart_putinteger10(tmp[0]);
		simple_uart_putstring((const uint8_t *)"\n\r");
        return -5;
    }
