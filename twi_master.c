...
static bool twi_master_write(uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    NRF_TWI1->TXD = *data++;	// register address
...
bool twi_master_transfer(uint8_t address, uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    if((0x02 & NRF_TWI1->ERRORSRC) || (0x04 & NRF_TWI1->ERRORSRC))
    {
    	NRF_TWI1->ERRORSRC=0;
    	simple_uart_putstring((const uint8_t *)"\n\rError>> NACK received(0x02=address NACK,0x04=data NACK,0x06=both) : ");
    	simple_uart_putinteger16(NRF_TWI1->ERRORSRC);
    	simple_uart_putstring((const uint8_t *)"\n\r");
    	return false;
    }
    ...
    {
        NRF_TWI1->ADDRESS = (address >> 1);	//7bit address
